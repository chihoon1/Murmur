...
for i  = 1: nfi
    ...
    % store patient_ID in the first column of H
    H(i,1) = Patient_ID;
    
    % compute entropy for each window
    for j = 1: length(yi)   
        % Estimate entropy
        h = wavelet_entropy(yi{j}, filt, L);
        H(i,j+1) = h;
    end
    Loc{i} =  Murmer_Location;
end  

%% compute coefficient of variation of entropy
D = [];
for i = 1 :size(H,1)
    a = H(i, 2:end);
    
    D = [D std(a(find(a)))/mean(a(find(a)))] ;
...
%% Seperate cases and control data from training dataset
...
%% Plot coefficient of variation of entropy without considering location
...
for i = 1:1
    ...
    xlabel('Coefficient of Variation of Entropy','fontsize',14 )
...
%saveas(fig,'Figures/Window_Entropy.png')

%% Save entropy into .csv files
Window_E.Case = X_case; Window_E.Control = X_control; Window_E.Un = X_un;
%save('Window_E.mat','Window_E')

writematrix( X_case,'./case_features/Window_Entropy_Case.csv');
writematrix( X_control,'./control_features/Window_Entropy_Control.csv');
% writematrix( X_un,'Window_Entropy_Un.csv');
%% Coeffcient of variation of entropy with respect to murmur locations
...
% find case and control patient IDs
A = [];
